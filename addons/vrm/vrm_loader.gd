...
func import_scene(path: String, flags: int, bake_fps: int, use_tmp: bool = false):
	...
	if type_exists("GLTFState") and type_exists("PackedSceneGLTF"):
		...
		root_node = gltf.import_gltf_scene(path, 0, 1000.0, compression_flags, gstate)
	else:
		...
		root_node = gltf.import_gltf_scene(path, 0, 1000.0, gstate)
...
func import_vrm(path:String):
	return import_scene(path, 0, 1000.0)
